...
public:
	...
	*/
	void CutOut(SimpleBitmapTemplate <ComponentType,NumComponentPerPixel> &destination,int thisX0,int thisY0,int wid,int hei,ComponentType clearColor) const
	{
		...
		destination.CleanUp();
		destination.bmpPtr = new ComponentType[GetTotalNumComponent(hei,wid)];
		destination.nx = wid;
		destination.ny = hei;

		ComponentType *desPtr;
		ComponentType *srcPtr;
		for(int ix=thisX0; ix<thisX0+wid; ++ix)
			for(int iy=thisY0;iy<thisY0+hei;++iy)
		{	
			if (IsInRange(ix, iy))
			{	
				srcPtr = bmpPtr+NumComponentPerPixel*(iy*this->nx+ix);
				desPtr = destination.GetEditablePixelPointer(ix-thisX0,iy-thisY0);
				desPtr[0] = srcPtr[0];
				desPtr[1] = srcPtr[1];
				desPtr[2] = srcPtr[2];
				desPtr[3] = srcPtr[3];
