...
/* virtual */ void FsLazyWindowApplication::Draw(void)
{
	...
	glBegin(GL_LINES);
	// glColor3ub(0,0,0);
	for (int i=0; i<airFoil.size()-1; i+=1)
	{
		
		
		// printf("Point 1: %f,%f,%f\n",airFoil[i][0], airFoil[i][1],airFoil[i][2]);
		// printf("Point 2: %f,%f,%f\n",airFoil[i+2][0], airFoil[i+2][1],airFoil[i+2][2]);
		glVertex3d(airFoil[i][0], airFoil[i][1],airFoil[i][2]);
		glVertex3d(airFoil[i+2][0], airFoil[i+2][1],airFoil[i+2][2]);
